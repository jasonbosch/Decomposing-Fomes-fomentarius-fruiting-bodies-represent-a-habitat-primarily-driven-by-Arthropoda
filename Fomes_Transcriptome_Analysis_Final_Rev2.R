#Analysis for: Decomposition of Fomes fomentatius fruiting bodies – transition of healthy living fungus into a decayed bacteria-rich habitat is primarily driven by Arthropoda 
#Jason Bosch
...
setwd("~/PostDoc/02_Projects/05_Fomes_transcription/04_Analysis_Results/R_Final_Rev2/")
...
sampleData_RNA$Condition <- gsub("Fresh","Healthy Living",sampleData_RNA$Condition)
...
names(condition_colours) <- c("Healthy Living","Rotten")
...
sampleData_RNA_FIXED[sampleData_RNA_FIXED$Sample=="RNA_H08","Condition"] <- "Healthy Living"
rawCounts_RNA_DF_FIXED[rawCounts_RNA_DF_FIXED$Sample=="RNA_H08","Condition"] <- "Healthy Living"

#Recode 8 and 22 as fresh for DNA
sampleData_DNA_FIXED <- sampleData_DNA
sampleData_DNA_FIXED[sampleData_DNA_FIXED$Sample=="DNA_H08","Condition"] <- "Healthy Living"
sampleData_DNA_FIXED[sampleData_DNA_FIXED$Sample=="DNA_H22","Condition"] <- "Healthy Living"
...
Fresh_Families <- as.data.frame(aggregate(.~TAX_Family, Fomes_tidy_shrunk_DNA_NoH19[Fomes_tidy_shrunk_DNA_NoH19$Condition=="Healthy Living",c("TAX_Family","TPM")],sum))
...
for (TERM_2 in c("CAZ_Family", "CAZ_Subfamily", "CAZ_target")) {
  ...
  fresh_table <- as.data.frame(table(Tidy_CAZymes_RNA[Tidy_CAZymes_RNA$Condition=="Healthy Living"&Tidy_CAZymes_RNA$TPM>0,TERM_2],Tidy_CAZymes_RNA[Tidy_CAZymes_RNA$Condition=="Healthy Living"&Tidy_CAZymes_RNA$TPM>0,"TAX_Family"]))
  colnames(fresh_table) <- c(TERM_2,"TAX_Family","Healthy_Living_Genes")
  ...
  merged_table[is.na(merged_table$Fresh_Genes),"Healthy_Living_Genes"] <- 0
  ...
  #Sort
  merged_table <- merged_table[,c(TERM_2,"TAX_Phylum","TAX_Class","TAX_Order","TAX_Family","Healthy_Living_Genes","Rotten_Genes")]
  ...
  for (ROW in 1:nrow(merged_table)) {
    merged_table[ROW,"Healthy_Living_TPM"] <- round(sum(Tidy_CAZymes_RNA[Tidy_CAZymes_RNA[,TERM_2]==merged_table[ROW,TERM_2] & 
                                                        Tidy_CAZymes_RNA$TAX_Family==merged_table[ROW,"TAX_Family"] &
                                                        Tidy_CAZymes_RNA$Condition=="Healthy Living","TPM"]),digits = 3)
...
colnames(MG_Bact_Families_Combined) <- c("TAX_Family","TPM_Healthy_Living","Percentage_Healthy_Living","TPM_Rotten","Percentage_Rotten")
...
MG_Bact_Families_Combined <- MG_Bact_Families_Combined[,c("TAX_Phylum","TAX_Class","TAX_Order","TAX_Family","TPM_Healthy_Living","Percentage_Healthy_Living","TPM_Rotten","Percentage_Rotten")]
MG_Bact_Families_Combined_Sub <- MG_Bact_Families_Combined[MG_Bact_Families_Combined$Percentage_Healthy_Living>1 | MG_Bact_Families_Combined$Percentage_Rotten>1,]
...
MG_Bact_Phylum_Abundance <- merge.data.frame(aggregate(x = MG_Bact_Families_Combined[,c("TAX_Phylum","Percentage_Healthy_Living")],by = .~TAX_Phylum, sum),
                 aggregate(x = MG_Bact_Families_Combined[,c("TAX_Phylum","Percentage_Rotten")],by = .~TAX_Phylum, sum))

#Export table
#Combine manually for display
write.table(MG_Bact_Pathways_Fresh,file = "MG_Bact_Pathways_Healthy_Living.csv",quote = T,row.names = FALSE,sep = ",")
...
# [1] yulab.utils_0.0.8  utf8_1.2.3         generics_0.1.3     stringi_1.7.12     lattice_0.21-8     magrittr_2.0.3     grid_4.3.3        
# [8] fastmap_1.1.1      Matrix_1.6-1       fansi_1.0.4        scales_1.2.1       textshaping_0.3.7  cli_3.6.1          rlang_1.1.1       
# [15] munsell_0.5.0      withr_2.5.0        cachem_1.0.8       tools_4.3.3        parallel_4.3.3     memoise_2.0.1      dplyr_1.1.3       
# [22] colorspace_2.1-0   vctrs_0.6.3        R6_2.5.1           gridGraphics_0.5-1 lifecycle_1.0.3    MASS_7.3-60        ragg_1.2.7        
# [29] pkgconfig_2.0.3    pillar_1.9.0       gtable_0.3.4       glue_1.6.2         Rcpp_1.0.11        systemfonts_1.0.5  tibble_3.2.1      
# [36] tidyselect_1.2.0   rstudioapi_0.15.0  farver_2.1.1       svglite_2.1.3      labeling_0.4.3     compiler_4.3.3    